      ...
      sub.subcontractors(job_id).each do |i|
        ...
        unless sub.suppliers(job_id).first.nil?
          sub.suppliers(job_id).each do |j|
            j.state = (Assignment.find_by_job_id_and_parent_id_and_partner_id_and_partner_type(job_id, sub.id, j.id, 2).checklist_items.collect {|k| k.state}.flatten).sort!.first
            if j.state.nil?
              j.state = 4
            end            
          end
